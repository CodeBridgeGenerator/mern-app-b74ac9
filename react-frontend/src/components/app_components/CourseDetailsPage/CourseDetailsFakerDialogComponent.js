...
import courseDetailsFakerFactory from "./courseDetailsFakerFactory";
...
const CourseDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = courseDetailsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("courseDetails").create(elem));
...
export default connect(mapState, mapDispatch)(CourseDetailsFakerDialogComponent);
