...
const CourseDetailsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            courseId: _entity?.courseId,courseName: _entity?.courseName,coordinator: _entity?.coordinator,
    ...
    return (
        <Dialog header="Create CourseDetails" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
